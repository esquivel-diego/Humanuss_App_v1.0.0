...
import { getAttendanceLast8DaysAllIns } from "@services/attendanceRangeService"
...
const AttendanceTable = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (!user) return
      try {
        const data = await getAttendanceLast8DaysAllIns()
        setWeek(data)
      } catch (err) {
          ...
          <table className="min-w-full table-auto divide-y divide-gray-200 dark:divide-gray-700">
            <thead className="bg-gray-100 dark:bg-gray-700 text-left">
              <tr>
                ...
                <th className="px-6 py-3 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-300">Check-Ins</th>
                ...
                week.map((entry, i) => (
                  <tr key={i} className="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                    ...
                    <td className="px-6 py-4 text-sm">
                      {entry.checkIns.length > 0 ? entry.checkIns.join(", ") : "--:--"}
                    </td>
                    <td className="px-6 py-4 text-sm">{entry.lastCheckOut || "--:--"}</td>
                  </tr>
                ))
              )}
            </tbody>
